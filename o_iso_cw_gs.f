      ...
      integer iry
      double precision rwt(3,nry), d18Oc(3,nry), fb
!---------------------------------------------------      
     ...
     $ 'o_iso_cw_cdf.csv',status = 'replace')
      ...
      open (90, file ='C:/Users/YK/Desktop/Isotope/'//
     $ 'o_iso_cw+ht_Wallmann.dat',status = 'replace')
      open (92, file ='C:/Users/YK/Desktop/Isotope/'//
     $ 'o_iso_cw+ht_Wallmann_2.dat',status = 'replace')
     ...
     $ 'Royer2014.txt',status = 'old')
      ...
      open(60, file = 'C:/Users/YK/Desktop/Isotope/rainwater.txt'
     $   ,status = 'old')
      read(60,*) rwt
      close(60)
...
!       fd(:) = 1.0d0
      ...
      do iry = 1, nry
      write(90,*) -5.92d8*fsr(iry),1.89d9*fsr(iry)
     $   ,0.1198d9*fsr(iry),-1.4233d9*fsr(iry)
      write(92,*) -5.92d8*rco2(iry)**0.5d0*fd(iry)
     $   *fa(iry)*(1.0d0-0.75d0*fl(iry))
     $ ,1.89d9*rco2(iry)**0.5d0*fd(iry)*fa(iry)*(1.0d0-0.75d0*fl(iry))
     $   ,0.1198d9*rco2(iry)**0.5d0*fd(iry)
     $  *fa(iry)*(1.0d0-0.75d0*fl(iry))
     $  ,-1.4233d9*rco2(iry)**0.5d0*fd(iry)
     $  *fa(iry)*(1.0d0-0.75d0*fl(iry))
      end do
...
!       k1 = 1d-11*10.0d0**(5.0d0*(i3-1.0d0)/(n3-1.0d0))
      ...
      k1 = 10.0d0**(-8.5d0)
C       k1 = 10.0d0**(-8.50d0)*rco2(iry)**0.5d0*fd(iry)   !! assuming constant rco2 dependence
