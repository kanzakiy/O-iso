      ...
      double precision rco2(nry), temp(nry), tp(nry), frw(nry)
      double precision fd(nry)
      integer iry
      
      integer is
C       integer, parameter :: ns = 8   !  for ff
C       integer, parameter :: ns = 3   !  for fb
C       integer, parameter :: ns = 2   !  for fl
...
!       rco2(:) = mry(20,:)
      ...
      write(*,*) is
      trestd = 10.0d0**(5.50d0) * 10.0d0**(2.0d0*(is-1.0d0)/(ns-1.0d0))
...
!       iry = nry
      ...
      kref = 10.0d0**(-8.50d0)*rco2(iry)**0.25d0
      end if 
      end if 
