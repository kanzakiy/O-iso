      ...
      double precision rt(nz),rtav,rwtav
      ...
      double precision rco2(nry), temp(nry), tp(nry), frw(nry), fd(nry)
      integer iry
      
      integer i5, rem
      integer, parameter :: n5 = 13
      
      double precision dsw(2,nry), drpmx(6,nry), drpmn(6,nry)
...
C       integer, parameter :: ns = 2  ! fl
      ...
      double precision :: f2std = 1d13
      double precision :: f3std = 1d13
      double precision :: trestd = 1d6
     ...
     $  'o_iso_sw_d18O-lim_sense.txt', 
     $                             status='replace')
      
      open (13, file='C:/Users/YK/Desktop/Isotope/'//
     $  'o_iso_sw_d18O-temp(dk)_sense.txt', 
     $                             status='replace')
      
      open (14, file='C:/Users/YK/Desktop/Isotope/'//
     $  'o_iso_sw_d18O-temp(gb)_sense.txt', 
     $                             status='replace')
      
      
      open(60, file = 'C:/Users/YK/Desktop/Isotope/seawater-list.txt'
     $   ,status = 'old')
      ...
      write(*,*) is
      T2 = 200.0d0 + 150.0d0*(is-1.0d0)/(ns-1.0d0)
      ...
      write(*,*) is
      T3 = 350.0d0 + 250.0d0*(is-1.0d0)/(ns-1.0d0)
      ...
      write(*,*) is
      f2std = 1d12 * 10.0d0**(2.0d0*(is-1.0d0)/(ns-1.0d0))
      ...
      write(*,*) is
      f3std = 1d12 * 10.0d0**(2.0d0*(is-1.0d0)/(ns-1.0d0))
      ...
      write(*,*) is
      trestd = 10.0d0**(5.5d0) * 10.0d0**(2.0d0*(is-1.0d0)/(ns-1.0d0))
...
!       iry = nry
      ...
      if (flg_fb == 1) then 
      if (is == 2) then 
      kref = 10.0d0**(-8.50d0)*rco2(iry)**0.50d0
      else if (is == 3) then 
      kref = 10.0d0**(-8.50d0)*rco2(iry)**0.250d0
      end if 
      end if
      ...
      if (flg_tau == 1) then 
      if (is == 1) then
      tres = trestd/fsr(iry)   ! std (reactive area is const)
      else if (is == 2) then
      tres = trestd
      end if 
      end if 
      ...
      T1 = 5.0d0 + 1.9d0*(temp(iry) -temp(nry))
        ...
        ! ------------------------------------------------------------  constant fraction of heat flow
        
        f1=1d16*(fsp/3.0d0)!**0.50d0
        f2=(200.0d0*f2std/T2)*(fsp/3.0d0)!**0.50d0
        f3=(350.0d0*f3std/T3)*(fsp/3.0d0)!**0.50d0
        
        ! ------------------------------------------------------------ not constant fraction of heat flow
        
        ! f1=1d16*(fsp/3.0d0)!**0.50d0
        ! f2=(f2std)*(fsp/3.0d0)!**0.50d0
        ! f3=(f3std)*(fsp/3.0d0)!**0.50d0
...
!      $  (1d3*(r3(iz)/rsmow/(1.0d0-r3(iz))-1.0d0),iz=1,nz,40)
      ...
      r1av = 1d3*(r1av/(1.0d0-r1av)/rsmow - 1.0d0)
      rw1av = 1d3*(rw1av/(1.0d0-rw1av)/rsmow - 1.0d0)
      r2av = 1d3*(r2av/(1.0d0-r2av)/rsmow - 1.0d0)
      rw2av = 1d3*(rw2av/(1.0d0-rw2av)/rsmow - 1.0d0)
      r3av = 1d3*(r3av/(1.0d0-r3av)/rsmow - 1.0d0)
      rw3av = 1d3*(rw3av/(1.0d0-rw3av)/rsmow - 1.0d0)
...
!       write(*,*) r1av(it),r2av(it),r3av(it)
     ...
     $  ,'@','(',is1,',',is2,',',is3,',',is4,')'     
      ...
      drpmn(1,iry) = min(drpmn(1,iry),r1av)
      drpmn(2,iry) = min(drpmn(2,iry),r2av)
      drpmn(3,iry) = min(drpmn(3,iry),r3av)
      drpmn(4,iry) = min(drpmn(4,iry),rw1av)
      drpmn(5,iry) = min(drpmn(5,iry),rw2av)
      drpmn(6,iry) = min(drpmn(6,iry),rw3av)
      
      drpmx(1,iry) = max(drpmx(1,iry),r1av)
      drpmx(2,iry) = max(drpmx(2,iry),r2av)
      drpmx(3,iry) = max(drpmx(3,iry),r3av)
      drpmx(4,iry) = max(drpmx(4,iry),rw1av)
      drpmx(5,iry) = max(drpmx(5,iry),rw2av)
      drpmx(6,iry) = max(drpmx(6,iry),rw3av)
      ...
      do iry = 1, nry
      write(12,*) tp(iry), drpmn(1,iry),drpmn(2,iry),drpmn(3,iry)
     $   ,drpmn(4,iry),drpmn(5,iry),drpmn(6,iry)
      end do
      do iry = nry, 1, -1
      write(12,*) tp(iry), drpmx(1,iry),drpmx(2,iry),drpmx(3,iry)
     $   ,drpmx(4,iry),drpmx(5,iry),drpmx(6,iry)
      end do
      
      
      write(12,*) tp(1), drpmn(1,1),drpmn(2,1),drpmn(3,1)
     $   ,drpmn(4,1),drpmn(5,1),drpmn(6,1)
      
      write(13,*) T2, 0.50d0*(maxval(drpmn(5,:))+minval(drpmn(5,:)))
     $    , 0.50d0*(maxval(drpmn(5,:))-minval(drpmn(5,:)))
      
      write(14,*) T3, 0.50d0*(maxval(drpmn(6,:))+minval(drpmn(6,:)))
     $    , 0.50d0*(maxval(drpmn(6,:))-minval(drpmn(6,:)))
